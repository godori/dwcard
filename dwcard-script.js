...
let game = {
  ...
  initGame: () => {
    ...
    $(".card")
      ...
      .on("mousemove", e => {
        ...
        let cardImg = card.style.backgroundImage;
        ...
        for (let i = $tooltip.length; i--; ) {
          ...
          let dt = $(tooltip).find("tooltip-body .img")[0];
          $(tooltip)
            .find("tooltip-header .name")
            .text(name);
          $(tooltip)
            .find("tooltip-body .text .desc")
            .text(desc);
          $(tooltip)
            .find("tooltip-body .text .ability")
            .text(ability);
  ...
  _calculateScore: owner => {
    ...
    let cards = $("." + owner + "-area .card-play .card");
    let sum = 0;
  ...
  faction: {
    ...
    setOpponent: () => {
      ...
      const imgUrl =
        "img/faction-cover/min/" + game.factionList[randNum] + "-faction.png";
  ...
  showFactionDesc: target => {
    $(target).hover(() => {
      const faction = $(target).data("factionValue");
  ...
  moveCard: target => {
    let card = $(target);
    const playLine = $(".player-board .card-play");

    let oldOffset = card.offset();
    card.appendTo(playLine);
    let newOffset = card.offset();

    let temp = card.clone().appendTo("body");
    temp.css({
      position: "absolute",
      left: oldOffset.left,
      top: oldOffset.top,
  ...
  _decorateCard: $card => {
    if (!$card.data("isOpen")) return;

    const point = $card.data("point");
    let $point = $('<div class="point"></div>').text(point);
    $card.append($point);
  ...
  _setCardImage: card => {
    ...
    const coverImg =
      "img/faction-cover/min/" + card.data("faction") + "-faction.png";
